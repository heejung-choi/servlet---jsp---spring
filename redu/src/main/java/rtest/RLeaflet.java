...
public class RLeaflet {	
	...
	public String returnLeaflet2(String path)  {
		...
		try {
			...
			r.eval("lan <- mk$lon");
			r.eval("lat <- mk$lat");
			r.eval("msg <- '<strong><a href=http://www.multicampus.co.kr style=text-decoration:none >멀티캠퍼스</a></strong><hr>우리가 공부하는 곳'");//팝업 윈도우 출력했을때 문장
			r.eval("map2 <- leaflet() %>% setView(lng = mk$lon, lat = mk$lat, zoom = 14) %>% addTiles() %>% addCircles(lng = lan, lat = lat, color='green', popup = msg )");
			//map2를 출력하시오 라고 하면 뷰어창에 원래 출력이 되는데(R에서), 여기에서는 최종결과를 saveWidget에 저장하게끔 하고 있다.
			String fileName = path + "/index.html";
