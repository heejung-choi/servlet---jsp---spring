...
public class RGraphController2 {
	...
	public ModelAndView get9(HttpServletRequest req) {//httpserveletrequest 정보를 전달해줘
		ModelAndView mav = new ModelAndView();
		String real_path = req.getSession().getServletContext().getRealPath("/"); //실제 path
		System.out.println(real_path);
	    real_path = real_path.replace("\\", "/");
	    System.out.println(real_path);		
		...
		/*
		 * http://localhost:8000/redu/resources/leafletchart2/index.html
		 * 실제 우리가 이렇게 저장한 것이 아니라 톰캣서버에 저장되 있는 것이다 요청하면 요청 받아지긴 한다.
		 */		mav.setViewName("leafltview");
		return mav;
	}	
