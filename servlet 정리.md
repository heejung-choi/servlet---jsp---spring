...
- HttpSession 객체를 이용한 방법
  ...
  - 모든 클라이언트 정보를 서버에 저장
  - 단점: 서버 업그레이드, 서버의 오버 헤드가 발생할 수 있다. 브라우저가 켜져 있는 동안 객체가 살아 있다.
  - session 객체는 살아있는 동안 한개만 살아있게 되어있다. server에는 세션객체가 여러개 있는데 이것은 세션 id로 구분한다. 이것은 쿠키 기술로 저장한다. 
  - 쿠키는 클라이언트에 저장하면 요청할 때마다 헤더에 보낸다. 클라이언트가 일정 시간(일반적으로 30분 정도: inactive interval time)
...
- 세션 객체:(배열이든, 다른 객체이든 무관하다.)
  ...
  - ex>Set-Cookie: JSESSIONID=CF8E665FA7ED4AB4196DF8BD9A7D9D9C; Path=/sedu; HttpOnly
  - 세션객체가 아무리 많아도, 유지해준다. 그러나 브라우저가 재기동 되면 사라진다.
...
- Query 문자열에 서버에 올리려는 파일의 내용을 포함시켜서 웹서버 프로그램을 요청하는 것 
...
HTML 이전에는 파일을 한개만 선택 가능했음 

<input type = "file" mutilple>을 사용해서 멀티파일을 2개이상의 파일을 업로드할 수 있다.
...
파일 업로드시 기본적인 Query 문자열의 인코딩 규칙이 적용되면 안된다.

업로드 되는 파일도 Query 문자열로 서버에 전송되어야 하지만  Query 문자열의 인코딩 규칙으로 인코딩 되면 안된다. 

```html
<form method="POST" action=""> enctype="multipart/form-data"
    파일 업로드시에는 post 방식으로만 해야 한다. enctype="multipart/form-data"을 입력해줘야 한다.
</form>
```
